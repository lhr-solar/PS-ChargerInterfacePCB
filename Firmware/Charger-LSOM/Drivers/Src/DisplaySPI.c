...
static void Display_CS_Low(void) { HAL_GPIO_WritePin(DISPLAY_NSS_PORT, DISPLAY_NSS_PIN, GPIO_PIN_RESET); }
static void Display_CS_High(void) { HAL_GPIO_WritePin(DISPLAY_NSS_PORT, DISPLAY_NSS_PIN, GPIO_PIN_SET); }
static void Display_A0_Low(void) { HAL_GPIO_WritePin(DISPLAY_MISO_PORT, DISPLAY_MISO_PIN, GPIO_PIN_RESET); } // A0=0: command
static void Display_A0_High(void) { HAL_GPIO_WritePin(DISPLAY_MISO_PORT, DISPLAY_MISO_PIN, GPIO_PIN_SET); }  // A0=1: data
...
{
    Display_RES_High();
    vTaskDelay(pdMS_TO_TICKS(DISPLAY_RESET_LOW_DELAY_MS));
    Display_RES_Low();
    vTaskDelay(pdMS_TO_TICKS(DISPLAY_RESET_HIGH_DELAY_MS));
    Display_RES_High();
    vTaskDelay(pdMS_TO_TICKS(DISPLAY_STARTUP_DELAY_MS));
}

static void Display_Update_Internal(void)
{
...
static void Display_Clear_Internal(void)
{
    memset(framebuffer, 0x00, DISPLAY_BUF_SIZE); // 0x00: all bits low = all pixels off
...
void Display_GPIO_Init(void)
{
    ...
    HAL_GPIO_WritePin(DISPLAY_NSS_PORT, DISPLAY_NSS_PIN, GPIO_PIN_SET);     // CS = high (deselected)
    HAL_GPIO_WritePin(DISPLAY_RES_PORT, DISPLAY_RES_PIN, GPIO_PIN_SET);     // RES = high (not in reset)
    HAL_GPIO_WritePin(DISPLAY_MISO_PORT, DISPLAY_MISO_PIN, GPIO_PIN_RESET); // A0 = low (command mode by default)
    ...
    HAL_GPIO_Init(DISPLAY_MISO_PORT, &GPIO_InitStruct);
    ...
    HAL_GPIO_Init(DISPLAY_NSS_PORT, &GPIO_InitStruct);
}
...
void Display_SetPixel(uint8_t x, uint8_t y, bool on)
{
    ...
    uint16_t idx = (y / 8) * DISPLAY_WIDTH + x; // 8: each page is 8 pixels tall; divides y into page index
    uint8_t bit = y % 8;                        // 8: remainder gives the bit position within the page byte
...
Display_status_t Display_DrawString(uint8_t x, uint8_t y, const char *str)
{
    ...
    if (xSemaphoreTake(DisplayMutex, portMAX_DELAY) == pdTRUE)
    {
        ...
        {
            xSemaphoreGive(DisplayMutex);
            return DISPLAY_ERROR_OUT_OF_BOUNDS;
        }
...
{
    for (uint8_t i = 0; i < 10; i++) // 10: draws a 10-pixel wide X mark for visual alignment debug
    {
        Display_SetPixel(i, i, true);
        Display_SetPixel(i, (uint8_t)(9 - i), true); // 9: mirrors the index to form the other diagonal of the X (i + (9-i) = 9)
    }
    Display_Update();
}
